...
// Variable pour stocker l'animation en cours
let currentAnimation = null;

// Fonction de scramble text améliorée
function scrambleText(element, originalText, specialChars) {
  // Si une animation est en cours, on la tue et on restore le texte original
  if (currentAnimation) {
...
function initHomeHero() {
  ...
  tl.from(
    splitedTitle.lines,
    { opacity: 0.01, stagger: 0.2, ease: "none", duration: durationL },
    0.3
  );
  tl.from(
    splitedSubtitle.lines,
    { opacity: 0.01, ease: "none", duration: durationL, stagger: 0.2 },
    ">-0.5"
  );
  tl.from(btn, { opacity: 0.01, yPercent: 100 }, "< 0.3");
  tl.from(
    visual,
    { opacity: 0.01, ease: "none", duration: durationL },
    "< 0.2"
  );
}
...
function initHomeFeatures() {
  ...
  sections.forEach((section) => {
    ...
    let subtitle = section.querySelector(
      ".text-large.is--home-features-subtitle"
    );
...
function initHomeFeaturesFaq() {
  const wrappers = document.querySelectorAll('[data-m-faq="wrap"]');
  ...
  wrappers.forEach((item) => {
    const btn = item.querySelector('[data-m-faq="btn"]');
    const line = item.querySelector(".home-features__faq-icon-line");
    let isOpen = false;

    // Créer la timeline en dehors du click event
    const tl = gsap.timeline({ paused: true });
    gsap.set(item, { height: btn.offsetHeight });
    gsap.set(line, { opacity: 1 });

    // Définir l'animation une seule fois
    tl.to(item, {
      height: "auto",
      //duration: durationS, // Ajout de la durée manquante
      ease: easeInOut,
    });
    ...
    btn.addEventListener("click", () => {
      if (isOpen) {
        // Si c'est ouvert, on inverse l'animation
        tl.reverse();
      } else {
        // Si c'est fermé, on joue l'animation
        tl.play();
      }
      isOpen = !isOpen;
    });
...
function initHomeHeroSVG() {
  ...
  fetch(wrapper.data)
    ...
    .then((svgText) => {
      ...
      setTimeout(() => {
        let networkLogo = svgElement.querySelector(
          '[filter="url(#filter3_i_2916_19867)"]'
        );
        let networkLogoMask = svgElement.querySelector(
          '[mask="url(#mask0_2916_19867)"]'
        );
        let networkLogoLine = svgElement.querySelector(
          '[d="M673 193C673 195.916 675.363 198.28 678.277 198.28C681.192 198.28 683.555 195.916 683.555 193C683.555 190.084 681.192 187.72 678.277 187.72C675.363 187.72 673 190.084 673 193Z"]'
        );
        let terminalLogo = {
          fill: svgElement.querySelector(
            '[fill="url(#paint4_radial_2916_19867)"]'
          ),
          stroke: svgElement.querySelector('[stroke="#181818"]'),
          stroke2: svgElement.querySelector(
            '[stroke="url(#paint5_radial_2916_19867)"]'
          ),
          fill2: svgElement.querySelector('[fill="black"]'),
          filter: svgElement.querySelector(
            '[filter="url(#filter0_dddddiiiiii_2916_19867)"]'
          ),
          fill3: svgElement.querySelector('[fill="#0F0F0F"]'),
          fill4: svgElement.querySelector('[fill="#2E74FF"]'),
          fill5: svgElement.querySelector(
            '[fill="url(#paint6_radial_2779_27401)"]'
          ),
          stroke3: svgElement.querySelector(
            '[stroke="url(#paint7_linear_2779_27401)"]'
          ),
          stroke4: svgElement.querySelector(
            '[stroke="url(#paint8_linear_2779_27401)"]'
          ),
          fill6: svgElement.querySelector(
            '[fill="url(#paint9_linear_2779_27401)"]'
          ),
          fill7: svgElement.querySelector(
            '[fill="url(#paint10_linear_2779_27401)"]'
          ),
          filter2: svgElement.querySelector(
            '[filter="url(#filter1_f_2916_19867)"]'
          ),
          filter3: svgElement.querySelector(
            '[filter="url(#filter2_f_2916_19867)"]'
          ),
        };
        ...
        let followingPath2 = document.querySelector(
          '[stroke="url(#paint2_linear_2916_19867)"]'
        );
        followingPath2.setAttribute(
          "d",
          "M993 241.5H919.64C918.303 241.5 917.054 242.168 916.312 243.281L873.172 307.992C872.733 308.649 872.5 309.421 872.5 310.211V427.963C872.5 428.952 872.133 429.907 871.47 430.642L846.191 458.679C845.433 459.52 844.353 460 843.22 460H789.5"
        );
        let pathLength2 = followingPath2.getTotalLength();

        let followingPath3 = document.querySelector(
          '[stroke="url(#paint1_linear_2916_19867)"]'
        );
        followingPath3.setAttribute(
          "d",
          "M1078 291V344.976C1078 346.248 1078.61 347.445 1079.63 348.199L1118.44 376.723C1119.13 377.228 1119.96 377.5 1120.81 377.5H1173.97C1174.95 377.5 1175.91 377.136 1176.64 376.478L1208.17 348.192C1209.02 347.433 1209.5 346.351 1209.5 345.215V160.174C1209.5 159.103 1209.07 158.077 1208.31 157.325L1176.67 126.151C1175.92 125.413 1174.91 125 1173.86 125H831.5"
        );
        let pathLength3 = followingPath3.getTotalLength();

        let followingPath4 = document.querySelector(
          '[stroke="url(#paint0_linear_2916_19867)"]'
        );
        followingPath4.setAttribute(
          "d",
          "M362 291V344.976C362 346.248 361.394 347.445 360.369 348.199L321.557 376.723C320.87 377.228 320.04 377.5 319.188 377.5H266.031C265.045 377.5 264.094 377.136 263.36 376.478L231.829 348.192C230.983 347.433 230.5 346.351 230.5 345.215V160.174C230.5 159.103 230.93 158.077 231.693 157.325L263.332 126.151C264.08 125.413 265.089 125 266.14 125H608.5"
        );
        let pathLength4 = followingPath3.getTotalLength();

        gsap.set(followingPath1, {
          strokeDasharray: `0 ${pathLength}`,
          strokeDashoffset: 0,
        });
        gsap.set(followingPath2, {
          strokeDasharray: `0 ${pathLength2}`,
          strokeDashoffset: -pathLength2,
        });
        gsap.set(followingPath3, {
          strokeDasharray: `0 ${pathLength3}`,
          strokeDashoffset: -pathLength3,
        });
        gsap.set(followingPath4, {
          strokeDasharray: `0 ${pathLength3}`,
        ...
        tl.from(
          networkLogo,
          { opacity: 0, duration: durationL, ease: "none" },
          2.5
        );
        tl.from(
          networkLogoMask,
        ...
        gsap.set(
          brandsElements,
          { filter: "grayscale(100%)", duration: durationL, ease: "none" },
          8
        );
        ...
        dashTl.to(
          followingPath2,
          {
            strokeDasharray: `40 ${pathLength2}`,
            willChange: "transform",
            force3D: true,
          },
          7.8
        ...
        dashTl.to(
          followingPath3,
          {
            strokeDasharray: `40 ${pathLength3}`,
            willChange: "transform",
            force3D: true,
          },
          "<"
        ...
        dashTl.to(
          followingPath4,
          {
            strokeDasharray: `40 ${pathLength4}`,
            willChange: "transform",
            force3D: true,
          },
          "<"
        ...
        // Au début de setTimeout, ajoutez :
        let isTerminalHovering = false;
        let isTimelineActive = true;

        // Après la création de 'tl', ajoutez :
        tl.eventCallback("onComplete", () => {
          isTimelineActive = false;
        });

        // Stockez le texte original dès le début
        let btnText = btn.querySelector(".text-xsmall");
        ...
        function handleLeaveEffect() {
          if (!isTimelineActive) {
            gsap.to([terminalElements[7], terminalElements[8]], {
              opacity: 0.5,
            });
          }
        }
...
function initHomeHeroSVGMobile() {
  ...
  fetch(wrapper.data)
    ...
    .then((svgText) => {
      ...
      setTimeout(() => {
        let networkLogo = svgElement.querySelector(
          '[filter="url(#filter3_i_2900_24211)"]'
        );
        let terminalLogo = {
          fill: svgElement.querySelector(
            '[fill="url(#paint2_radial_2900_24211)"]'
          ),
          stroke: svgElement.querySelector('[stroke="#181818"]'),
          stroke2: svgElement.querySelector(
            '[stroke="url(#paint3_radial_2900_24211)"]'
          ),
          fill2: svgElement.querySelector('[fill="black"]'),
          filter: svgElement.querySelector(
            '[filter="url(#filter0_dddddiiiiii_2900_24211)"]'
          ),
          fill3: svgElement.querySelector('[fill="#0F0F0F"]'),
          fill4: svgElement.querySelector('[fill="#2E74FF"]'),
          fill5: svgElement.querySelector(
            '[fill="url(#paint4_radial_2900_24211)"]'
          ),
          stroke3: svgElement.querySelector(
            '[stroke="url(#paint5_linear_2900_24211)"]'
          ),
          stroke4: svgElement.querySelector(
            '[stroke="url(#paint6_linear_2900_24211)"]'
          ),
          fill6: svgElement.querySelector(
            '[fill="url(#paint7_linear_2900_24211)"]'
          ),
          fill7: svgElement.querySelector(
            '[fill="url(#paint8_linear_2900_24211)"]'
          ),
          filter2: svgElement.querySelector(
            '[filter="url(#filter1_f_2900_24211)"]'
          ),
          filter3: svgElement.querySelector(
            '[filter="url(#filter2_f_2900_24211)"]'
          ),
        };
        ...
        let terminalBtn = document.querySelector(
          '[fill="url(#paint2_radial_2900_24211)"]'
        );
        // Créer un tableau avec tous les éléments de terminalLogo
        const terminalElements = Object.values(terminalLogo).filter(
        ...
        let followingPath1 = document.querySelector(
          '[stroke="url(#paint1_linear_2900_24211)"]'
        );
        followingPath1.setAttribute(
          "d",
        ...
        let followingPath2 = document.querySelector(
          '[stroke="url(#paint0_linear_2900_24211)"]'
        );
        followingPath2.setAttribute(
          "d",
          "M305.542 447.978H273.957C273.381 447.978 272.844 448.266 272.524 448.745L253.95 476.607C253.761 476.889 253.661 477.222 253.661 477.562V528.261C253.661 528.687 253.503 529.097 253.217 529.414L242.333 541.485C242.007 541.847 241.542 542.054 241.054 542.054H217.925"
        );
        let pathLength2 = followingPath2.getTotalLength();
  ...
  fetch(wrapper.data)
    ...
    .then((svgText) => {
      ...
      setTimeout(() => {
        ...
        let networkLogo = {
          clipPath: svgElement.querySelector(
            '[clip-path="url(#clip0_2858_26399)"]'
          ),
        };
        let networkLogoMask = svgElement.querySelector(
          '[mask="url(#mask0_2858_26399)"]'
        );

        // Créer un tableau avec tous les éléments de terminalLogo
        const networkElements = Object.values(networkLogo).filter(
          (el) => el !== null
        );

        mm.add("(max-width: 479px)", () => {
          gsap.set(networkElements, {
            rotateZ: "90deg",
            xPercent: 102.5,
            scale: 1,
          });
        ...
        let followingPath1 = document.querySelector(
          '[stroke="url(#paint0_linear_2858_26399)"]'
        );
        followingPath1.setAttribute(
          "d",
          "M643.301 238.61H586.012C584.274 238.61 582.622 237.857 581.482 236.545L559.499 211.243C558.55 210.152 558.028 208.754 558.028 207.308V167.723C558.028 166.424 557.607 165.161 556.828 164.123L539.82 141.445C538.686 139.934 536.908 139.045 535.02 139.045H389"
        );
        let pathLength1 = followingPath1.getTotalLength();

        let followingPath2 = document.querySelector(
          '[stroke="url(#paint1_linear_2858_26399)"]'
        );
        followingPath2.setAttribute(
          "d",
          "M796.222 238.61H853.512C855.25 238.61 856.902 237.857 858.041 236.545L880.025 211.243C880.974 210.152 881.496 208.754 881.496 207.308V167.723C881.496 166.424 881.917 165.161 882.696 164.123L899.704 141.445C900.838 139.934 902.616 139.045 904.504 139.045H1050"
        );
        let pathLength2 = followingPath2.getTotalLength();

        let followingPath3 = document.querySelector(
          '[stroke="url(#paint2_linear_2858_26399)"]'
        );
        followingPath3.setAttribute(
          "d",
          "M643.301 284.819H586.012C584.274 284.819 582.622 285.573 581.482 286.884L559.499 312.186C558.55 313.278 558.028 314.676 558.028 316.122V355.707C558.028 357.005 557.607 358.268 556.828 359.307L539.82 381.985C538.686 383.496 536.908 384.385 535.02 384.385H391"
        );
        let pathLength3 = followingPath3.getTotalLength();

        let followingPath4 = document.querySelector(
          '[stroke="url(#paint3_linear_2858_26399)"]'
        );
        followingPath4.setAttribute(
          "d",
          "M796.222 284.819H853.512C855.25 284.819 856.902 285.573 858.041 286.884L880.025 312.186C880.974 313.278 881.496 314.676 881.496 316.122V355.707C881.496 357.005 881.917 358.268 882.696 359.307L899.704 381.985C900.838 383.496 902.616 384.385 904.504 384.385H1050.5"
        );
        let pathLength4 = followingPath3.getTotalLength();

        let followingPath5 = document.querySelector(
          '[stroke="url(#paint4_linear_2858_26399)"]'
        );
        followingPath5.setAttribute("d", "M644 261L285 261");
        let pathLength5 = followingPath3.getTotalLength();

        let followingPath6 = document.querySelector(
          '[stroke="url(#paint5_linear_2858_26399)"]'
        );
        followingPath6.setAttribute("d", "M795 261L1154 261");
        let pathLength6 = followingPath3.getTotalLength();

        gsap.set(followingPath1, {
          strokeDasharray: `0 ${pathLength1}`,
        ...
        gsap.set(followingPath5, {
          strokeDasharray: `0 ${pathLength3}`,
          strokeDashoffset: -pathLength5,
        });
        gsap.set(followingPath6, {
          strokeDasharray: `0 ${pathLength3}`,
          strokeDashoffset: -pathLength6,
        });

        let tl = gsap.timeline({ paused: true });
        let dashTl = gsap.timeline({
          paused: true,
          repeat: -1,
          repeatDelay: 0.5,
        });
        ...
        dashTl.to(followingPath1, {
          strokeDasharray: `40 ${pathLength1}`,
          willChange: "transform",
          force3D: true,
        });
        dashTl.to(
  ...
  fetch(wrapper.data)
    ...
    .then((svgText) => {
      ...
      setTimeout(() => {
        ...
        let networkLogo = {
          clipPath: svgElement.querySelector(
            '[clip-path="url(#clip0_2864_34569)"]'
          ),
        };
        let networkLogoMask = svgElement.querySelector(
          '[mask="url(#mask0_2864_34569)"]'
        );
        ...
        let followingPath1 = document.querySelector(
          '[stroke="url(#paint0_linear_2864_34569)"]'
        );
        followingPath1.setAttribute("d", "M283 284H713");
        let pathLength1 = followingPath1.getTotalLength();

        let followingPath2 = document.querySelector(
          '[stroke="url(#paint1_linear_2864_34569)"]'
        );
        followingPath2.setAttribute("d", "M734 284L1159 284");
        let pathLength2 = followingPath2.getTotalLength();

        let followingPath3 = document.querySelector(
          '[stroke="url(#paint2_linear_2864_34569)"]'
        );
        followingPath3.setAttribute("d", "M747 236L1158 236");
        let pathLength3 = followingPath3.getTotalLength();

        let followingPath4 = document.querySelector(
          '[stroke="url(#paint3_linear_2864_34569)"]'
        );
        ...
        mm.add("(max-width: 479px)", () => {
          ...
          gsap.set(svgElement.querySelectorAll('[fill="#161616"]'), {
            visibility: "hidden",
          });
          gsap.set(svgElement.querySelectorAll('[fill="#868686"]'), {
            visibility: "hidden",
          });
          gsap.set(
            svgElement.querySelector('[fill="url(#paint4_linear_2864_34569)"]'),
            { visibility: "hidden" }
          );
          gsap.set(svgElement.querySelectorAll('[stroke="#282828"]'), {
            visibility: "hidden",
          });
          gsap.set(svgElement, {
            rotateZ: -90,
